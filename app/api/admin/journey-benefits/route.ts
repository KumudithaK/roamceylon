...
export async function GET(request:Request){
  const actor=await authenticatedStaff(request,"benefits.view");if(!actor.authorized)return NextResponse.json({error:actor.status===401?"Unauthorized.":"You do not have permission to view journey benefits."},{status:actor.status});
...
export async function POST(request:Request){
  ...
  if(fulfilment.success){
    const actor=await authenticatedStaff(request,"operations.manage");if(!actor.authorized)return NextResponse.json({error:actor.status===401?"Unauthorized.":"You do not have permission to update benefit fulfilment."},{status:actor.status});
  ...
  const actor=await authenticatedStaff(request,"benefits.assign");if(!actor.authorized)return NextResponse.json({error:actor.status===401?"Unauthorized.":"You do not have permission to assign benefits."},{status:actor.status});
