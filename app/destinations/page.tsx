...
export default async function Page(){const items=await listContent("destinations");return <><ListingPage eyebrow="Explore the island" title="Places with a story to tell." copy="From sacred highlands to slow southern shores, every destination reveals a different Sri Lanka." basePath="/destinations" items={items.map(x=>({id:x.id,slug:x.slug,name:x.name,image:x.hero_image_url,alt:x.image_alt,eyebrow:x.province,description:x.short_description,unesco:Boolean(x.unesco_information)}))}/><div className="shell pb-24"><SriLankaMap destinations={items}/></div></>}
