...
export default async function Page({searchParams}:{searchParams:Promise<{theme?:string;themes?:string;destination?:string;experience?:string;step?:string;adults?:string;children?:string;infants?:string;experienceAdults?:string;experienceChildren?:string;experienceInfants?:string}>}){
  ...
  return <JourneyBuilder data={data} initialSelection={{themeId:query.theme,themeIds:query.themes?.split(",").filter(Boolean),destinationIds:query.destination?[query.destination]:undefined,experienceId:query.experience,step:query.step===undefined?undefined:number(query.step),travellers:hasTravellers?{adults:number(query.adults),children:number(query.children),infants:number(query.infants)}:undefined,experienceParticipants:hasExperienceParticipants?{adults:number(query.experienceAdults),children:number(query.experienceChildren),infants:number(query.experienceInfants)}:undefined}}/>;
}
