...
export function SiteFooter(){
  ...
  const [contact,setContact]=useState<Pick<Database["public"]["Tables"]["website_settings"]["Row"],"contact_phone"|"whatsapp_url"|"facebook_url"|"business_address"|"website_url"|"business_email">|null>(null);
  useEffect(()=>{void createClient().from("website_settings").select("contact_phone,whatsapp_url,facebook_url,business_address,website_url,business_email").eq("id",true).maybeSingle().then(({data})=>setContact(data))},[]);
