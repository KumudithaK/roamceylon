  ...
  return <>
    ...
    <section className="editorial-noise bg-forest py-20 text-ivory md:py-32">
      <div className="shell">
        ...
        <div className="mt-14 grid gap-x-5 gap-y-10 sm:grid-cols-2 lg:grid-cols-12">{themes.map((theme,index)=><FadeIn key={theme.id} delay={(index%4)*.05} className={index===0||index===5?"lg:col-span-6":"lg:col-span-3"}><Link href={`/discover/${theme.slug}`} className="image-lift group block focus-ring"><div className={`relative overflow-hidden bg-slate ${index===0||index===5?"aspect-[16/10]":"aspect-[3/4]"}`}>{theme.hero_image_url?<Image src={theme.hero_image_url} alt={theme.image_alt||editionDisplayName(theme)} fill sizes="(max-width: 640px) 100vw, 50vw" className="object-cover opacity-80"/>:null}<div className="absolute inset-0 bg-gradient-to-t from-forest via-transparent to-transparent"/><span className="absolute right-4 top-4 font-serif text-xl text-ivory/80">{String(index+1).padStart(2,"0")}</span><div className="absolute inset-x-0 bottom-0 p-5"><h3 className="font-serif text-2xl md:text-3xl">{editionDisplayName(theme)}</h3><p className="mt-2 line-clamp-2 text-xs leading-5 text-ivory/65">{theme.short_description}</p></div></div></Link></FadeIn>)}</div>
      </div>
    </section>
