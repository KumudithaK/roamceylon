...
export function HeroMedia({media}:{media:HeroMediaType}){
  ...
  const youtubeUrl=media.developmentYoutubePreviewId
    ?`https://www.youtube-nocookie.com/embed/${media.developmentYoutubePreviewId}?autoplay=1&mute=1&controls=0&loop=1&playlist=${media.developmentYoutubePreviewId}&start=0&end=63&playsinline=1&rel=0&disablekb=1`
    :null;
  ...
  return <div className="absolute inset-0">
    ...
    {showYoutubePreview&&youtubeUrl&&<div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
      <iframe src={youtubeUrl} title="Temporary development hero preview" className="absolute left-1/2 top-1/2 aspect-video min-h-full min-w-full -translate-x-1/2 -translate-y-[56%] scale-125 border-0" allow="autoplay; encrypted-media" referrerPolicy="strict-origin-when-cross-origin"/>
    </div>}
