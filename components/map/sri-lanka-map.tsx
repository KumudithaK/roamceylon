...
export function SriLankaMap({destinations,selectedIds=[],onSelect,className,mode="journey",destination,nearbyDestinations=[],journeyHref="/journey-builder"}:MapProps){
  ...
  const mapLabel=destinationMode?`Map showing ${destination?.name} and nearby Sri Lankan destinations`:`Map showing ${valid.length} destinations and ${route.length} selected stops`;
  ...
  return <section id={destinationMode?"destination-location":undefined} className={cn("grid overflow-hidden rounded-[2rem] bg-forest text-ivory md:grid-cols-[1fr_.75fr]",className)} aria-label={destinationMode?`${destination?.name} location map`:"Sri Lanka destinations map"}>
    ...
    {destinationMode&&destination
      ...
      :<JourneyPanel valid={valid} routeLength={route.length} selectedIds={selectedIds} onSelect={onSelect} estimatedDistance={estimatedDistance} estimatedTravelDays={estimatedTravelDays}/>
    }
  </section>;
}
