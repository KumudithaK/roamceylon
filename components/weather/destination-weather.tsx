...
export async function DestinationWeather({name,latitude,longitude,climate}:WeatherProps){
  ...
  return <article className="overflow-hidden rounded-[2rem] bg-forest p-7 text-ivory md:p-9">
    <div className="grid gap-8 lg:grid-cols-[300px_minmax(0,1fr)] lg:gap-10">
      <div className="flex flex-col justify-between"><div><div className="flex items-start justify-between gap-5"><div><p className="eyebrow text-gold-light">Weather in {name}</p>{forecast?<><div className="mt-4 flex items-end gap-3"><strong className="font-serif text-5xl font-normal">{forecast.temperature}°</strong><span className="pb-1 text-sm text-ivory/65">{forecast.condition}</span></div><div className="mt-4 flex flex-wrap gap-4 text-xs text-ivory/60">{forecast.humidity!==null?<span className="flex items-center gap-1.5"><Droplets className="size-4 text-gold-light"/>{forecast.humidity}% humidity</span>:null}{forecast.windSpeed!==null?<span className="flex items-center gap-1.5"><Wind className="size-4 text-gold-light"/>{forecast.windSpeed} km/h wind</span>:null}</div></>:<p className="mt-4 text-sm text-ivory/60">Live conditions are temporarily unavailable.</p>}</div>{forecast?<WeatherIcon symbol={forecast.symbol} className="size-11 text-gold-light"/>:<CloudSun className="size-11 text-gold-light"/>}</div></div>
        {climate?<div className="mt-7 border-t border-white/10 pt-6"><p className="text-[.65rem] font-bold uppercase tracking-widest text-gold-light">Seasonal outlook</p><p className="mt-3 text-sm leading-6 text-ivory/60">{climate}</p></div>:null}
      </div>
