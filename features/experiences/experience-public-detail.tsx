...
import {experienceContext} from "@/lib/experience-discovery";
...
function PlaceAndEdition({experience}:{experience:JourneyExperience}){
  ...
  return <section className="bg-sand"><div className="shell grid gap-10 py-14 md:grid-cols-2 md:py-20">
    {destinations.length?<div><MapPin className="size-6 text-gold" aria-hidden="true"/><p className="eyebrow mt-5">A sense of place</p><h2 className="mt-4 font-serif text-4xl">Part of your time in {destinations.map(item=>item.name).join(" and ")}.</h2><div className="mt-7 flex flex-wrap gap-3">{destinations.map(destination=><Link key={destination.id} href={`/destinations/${destination.slug}`} className="inline-flex min-h-11 items-center gap-2 border-b border-forest text-sm font-semibold">Discover {destination.name}<ArrowRight className="size-4"/></Link>)}</div></div>:null}
    {themes.length?<div><Compass className="size-6 text-gold" aria-hidden="true"/><p className="eyebrow mt-5">The Edition perspective</p><h2 className="mt-4 font-serif text-4xl">Ways this experience can shape your journey.</h2><div className="mt-7 flex flex-wrap gap-3">{themes.map(theme=><Link key={theme.id} href={`/discover/${theme.slug}`} className="inline-flex min-h-11 items-center border border-forest/30 px-4 py-2 text-sm font-semibold transition hover:border-forest hover:bg-ivory">{editionDisplayName(theme)}</Link>)}</div></div>:null}
  </div></section>;
}

function RelatedExperiences({experiences}:{experiences:JourneyExperience[]}){
  if(!experiences.length)return null;
  return <section className="shell py-16 md:py-24"><div className="grid gap-7 border-b border-forest/20 pb-8 md:grid-cols-[.55fr_1fr]"><p className="eyebrow">Continue discovering</p><h2 className="heading">Other moments that may belong in your journey.</h2></div><div className="mt-10 grid gap-8 md:grid-cols-2 lg:grid-cols-4">{experiences.slice(0,4).map(experience=><article key={experience.id}><Link href={`/experiences/${experience.slug}`} className="image-lift group block focus-ring"><ExperienceMedia src={experience.hero_image_url} alt={experience.image_alt||experience.name} sizes="(max-width: 768px) 100vw, 25vw" className="aspect-[4/3]" imageClassName="transition duration-1000 group-hover:scale-[1.035]"/><p className="mt-5 text-[.65rem] font-bold uppercase tracking-[.16em] text-gold">{experienceContext(experience)}</p><h3 className="mt-3 font-serif text-2xl leading-tight">{experience.name}</h3><span className="editorial-link mt-4 text-forest">Explore</span></Link></article>)}</div></section>;
}
...
export function ExperiencePublicDetail({experience,related}:{experience:JourneyExperience;related:JourneyExperience[]}){
  ...
  const facts=[["Duration",experience.duration],["Best considered",experience.best_season],["Experience",experience.category],["Place",experience.destinationNames?.join(" · ")]].filter((item):item is [string,string]=>Boolean(item[1]));
  return <article className="bg-ivory text-slate">
    ...
    <header className="relative isolate min-h-[68svh] overflow-hidden bg-forest text-ivory"><ExperienceMedia src={experience.hero_image_url} alt={experience.image_alt||experience.name} priority sizes="100vw" className="absolute inset-0" imageClassName="object-[center_45%]"/><div className="absolute inset-0 bg-gradient-to-t from-forest via-forest/35 to-forest/10"/><div className="shell relative flex min-h-[68svh] items-end py-12 md:py-20"><div className="max-w-5xl"><p className="eyebrow text-gold-light">{experienceContext(experience)}</p><h1 className="mt-5 max-w-5xl font-serif text-[clamp(3rem,7vw,6rem)] leading-[.98] tracking-[-.035em]">{experience.name}</h1>{experience.short_description?<p className="mt-6 max-w-2xl text-lg leading-8 text-ivory/82">{experience.short_description}</p>:null}</div></div></header>
    <section className="shell grid gap-10 py-14 md:grid-cols-[1.25fr_.75fr] md:py-24"><div><p className="eyebrow">The experience</p><h2 className="mt-5 max-w-3xl font-serif text-4xl leading-tight md:text-6xl">A moment to fold into the wider story of your journey.</h2></div>{facts.length?<dl className="divide-y divide-forest/20 border-y border-forest/20">{facts.map(([label,value])=><div key={label} className="grid grid-cols-[.7fr_1.3fr] gap-4 py-4"><dt className="text-xs font-bold uppercase tracking-[.14em] text-forest">{label}</dt><dd className="text-sm leading-6 text-muted">{value}</dd></div>)}</dl>:null}</section>
    <Narrative eyebrow="The story" title="Why this moment matters." copy={experience.full_description||experience.short_description}/>
    ...
    <ExperienceFooterCTA experience={experience} globalTravellers={journey.travellerCounts} initialParticipants={journey.experienceParticipants[experience.id]} initialPricingPlanId={journey.selectedPricingPlanIds[pricingPlanKey("experience",experience.id)]} included={selected} onInclude={include} onParticipantsChange={update}/>
