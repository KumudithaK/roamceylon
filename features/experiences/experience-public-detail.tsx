...
import {editionDisplayName,publicBrandText} from "@/lib/brand";
...
function RelatedExperiences({experiences}:{experiences:JourneyExperience[]}){
  ...
  return <section className="shell py-16 md:py-24"><div className="grid gap-7 border-b border-forest/20 pb-8 md:grid-cols-[.55fr_1fr]"><p className="eyebrow">Continue planning</p><h2 className="heading">More experiences to consider for your journey.</h2></div><div className="mt-10 grid gap-8 md:grid-cols-2 lg:grid-cols-4">{experiences.slice(0,4).map(experience=><article key={experience.id}><Link href={`/experiences/${experience.slug}`} className="image-lift group block focus-ring"><ExperienceMedia src={experience.hero_image_url} alt={publicBrandText(experience.image_alt||experience.name)} sizes="(max-width: 768px) 100vw, 25vw" className="aspect-[4/3]" imageClassName="transition duration-1000 group-hover:scale-[1.035]"/><p className="mt-5 text-[.65rem] font-bold uppercase tracking-[.16em] text-gold">{publicBrandText(experienceContext(experience))}</p><h3 className="mt-3 font-serif text-2xl leading-tight">{publicBrandText(experience.name)}</h3><span className="editorial-link mt-4 text-forest">Explore</span></Link></article>)}</div></section>;
}
...
export function ExperiencePublicDetail({experience,related}:{experience:JourneyExperience;related:JourneyExperience[]}){
  ...
  return <main className="bg-ivory text-slate">
    ...
    <header className="relative isolate min-h-[64svh] overflow-hidden bg-forest text-ivory"><ExperienceMedia src={experience.hero_image_url} alt={publicBrandText(experience.image_alt||experience.name)} priority sizes="100vw" className="absolute inset-0" imageClassName="object-[center_45%]"/><div className="absolute inset-0 bg-gradient-to-t from-forest via-forest/35 to-forest/10"/><div className="shell relative flex min-h-[64svh] items-end py-12 md:py-16"><div className="max-w-5xl"><p className="flex items-center gap-2 text-sm font-bold uppercase tracking-[.17em] text-gold-light"><MapPin className="size-4" aria-hidden="true"/>{publicBrandText(experiencePlace(experience))}</p>{editions.length?<p className="mt-3 text-[.68rem] font-bold uppercase tracking-[.17em] text-ivory/65">{editions.slice(0,2).join(" · ")}</p>:null}<h1 className="mt-5 max-w-5xl font-serif text-[clamp(2.65rem,5.4vw,5.3rem)] leading-[.98] tracking-[-.035em]">{publicBrandText(experience.name)}</h1>{experience.short_description?<p className="mt-6 max-w-2xl text-lg leading-8 text-ivory/82">{publicBrandText(experience.short_description)}</p>:null}<Button asChild variant="accent" size="lg" className="mt-8"><Link href="#plan-this-experience">Plan this into your journey<ArrowRight/></Link></Button></div></div></header>
    <section className="shell grid gap-10 py-14 md:grid-cols-[1.25fr_.75fr] md:py-24"><div><p className="eyebrow">How it fits your journey</p><h2 className="mt-5 max-w-3xl font-serif text-4xl leading-tight md:text-6xl">A distinctive experience, arranged as part of a journey that flows.</h2><p className="mt-6 max-w-2xl text-base leading-8 text-muted">Tell us what draws you to it. We’ll consider timing, routing and the other places that make sense around it.</p></div>{facts.length?<dl className="divide-y divide-forest/20 border-y border-forest/20">{facts.map(([label,value])=><div key={label} className="grid grid-cols-[.7fr_1.3fr] gap-4 py-4"><dt className="text-xs font-bold uppercase tracking-[.14em] text-forest">{label}</dt><dd className="text-sm leading-6 text-muted">{publicBrandText(value)}</dd></div>)}</dl>:null}</section>
