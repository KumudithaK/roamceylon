...
export function ExperienceCard({experience,onOpen,compact=false}:{experience:JourneyExperience;onOpen:()=>void;compact?:boolean}){
...
function JourneyTravellersModal({open,initial,onCancel,onConfirm}:{open:boolean;initial:ParticipantCounts;onCancel:()=>void;onConfirm:(counts:ParticipantCounts)=>void}){
  ...
  return <AnimatePresence>{open&&<motion.div className="fixed inset-0 z-[90] grid place-items-center bg-slate/70 p-4 backdrop-blur-sm" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={onCancel}><motion.div role="dialog" aria-modal="true" aria-labelledby="travellers-title" onMouseDown={event=>event.stopPropagation()} initial={{opacity:0,y:24,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:14}} className="w-full max-w-lg rounded-[2rem] bg-ivory p-7 text-slate shadow-2xl md:p-10"><p className="eyebrow">Whole trip</p><h2 id="travellers-title" className="mt-3 font-serif text-4xl">How many people are travelling on this trip?</h2><p className="mt-3 text-sm leading-6 text-slate/65">Enter everyone travelling with Roam Ceylon, including people who may skip this particular experience.</p><div className="mt-5 rounded-2xl border border-gold/25 bg-gold/10 p-4 text-sm leading-6"><strong className="block text-forest">Trip travellers and experience participants are different.</strong>You will choose who joins each experience separately.</div><div className="mt-6"><ExperienceParticipants counts={counts} onChange={setCounts} dark={false}/></div><div className="mt-7 flex justify-end gap-3"><Button variant="ghost" onClick={onCancel}>Not now</Button><Button disabled={total(counts)===0} onClick={()=>onConfirm(counts)}>Save trip travellers</Button></div></motion.div></motion.div>}</AnimatePresence>;
}
