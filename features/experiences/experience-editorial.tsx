...
export function ExperienceCard({experience,onOpen,compact=false,selected=false}:{experience:JourneyExperience;onOpen:()=>void;compact?:boolean;selected?:boolean}){
  return <motion.button layout onClick={onOpen} whileHover={{y:-6}} className={cn("group overflow-hidden rounded-[2rem] border-2 bg-white text-left transition",selected?"border-gold shadow-[0_24px_70px_rgba(193,140,45,.2)]":"border-transparent shadow-[0_24px_70px_rgba(26,40,35,.09)] hover:shadow-[0_30px_90px_rgba(26,40,35,.16)]")}>
...
export function ExperienceFooterCTA({experience,globalTravellers=emptyCounts,initialParticipants,onInclude,onRemove,onParticipantsChange,included=false}:{experience:JourneyExperience;globalTravellers?:ParticipantCounts;initialParticipants?:ParticipantCounts;onInclude?:(participants:ParticipantCounts,journeyTravellers:ParticipantCounts)=>void;onRemove?:()=>void;onParticipantsChange?:(participants:ParticipantCounts)=>void;included?:boolean}){
  ...
  const changeParticipants=(counts:ParticipantCounts)=>{setParticipants(counts);if(included&&withinTrip(counts,globalTravellers))onParticipantsChange?.(counts)};
  return <section className="bg-forest py-16 text-ivory md:py-24"><div className="shell grid items-center gap-10 md:grid-cols-[1.1fr_.9fr]"><div><p className="eyebrow text-gold-light">Make it part of your story</p><h2 className="mt-4 max-w-xl font-serif text-4xl leading-tight md:text-6xl">{included?"Already part of your journey.":"Design my journey with this experience."}</h2><p className="mt-5 max-w-xl leading-8 text-ivory/65">{included?"Participant changes update your journey immediately. Remove the experience whenever your plans change.":"Choose who will take part. This can be different from the total number travelling with you."}</p></div><div><ExperienceParticipants counts={participants} onChange={changeParticipants}/>{!validForTrip&&<p role="alert" className="mt-4 rounded-xl border border-gold-light/30 bg-gold/10 p-4 text-sm leading-6 text-gold-light">Experience participants cannot exceed your whole-trip travellers. Reduce these participants or increase the traveller count in the Plan step.</p>}<Button variant={included?"outline":"accent"} size="lg" className={cn("mt-6 w-full",included&&"border-gold-light/50 text-gold-light hover:bg-white/10")} disabled={!included&&(total(participants)===0||!validForTrip)} onClick={included?onRemove:include}>{included?"Remove from my journey":"Include this experience in my journey"}</Button></div></div><JourneyTravellersModal open={askingTravellers} initial={globalTravellers} minimum={participants} onCancel={()=>setAskingTravellers(false)} onConfirm={counts=>{setAskingTravellers(false);const next=total(participants)?participants:counts;setParticipants(next);onInclude?.(next,counts)}}/></section>;
}
...
function DetailContents({experience,related,onClose,globalTravellers,initialParticipants,onInclude,onRemove,onParticipantsChange,included,onRelatedOpen}:{experience:JourneyExperience;related:JourneyExperience[];onClose?:()=>void;globalTravellers?:ParticipantCounts;initialParticipants?:ParticipantCounts;onInclude?:(participants:ParticipantCounts,journeyTravellers:ParticipantCounts)=>void;onRemove?:()=>void;onParticipantsChange?:(participants:ParticipantCounts)=>void;included?:boolean;onRelatedOpen?:(experience:JourneyExperience)=>void}){
  return <article className="bg-ivory text-slate"><ExperienceHero experience={experience} onClose={onClose}/><ExperienceStory experience={experience}/><ExperienceGallery experience={experience}/><ExperienceHighlights experience={experience}/><EditorialSection eyebrow="Distinctly this place" title="What makes it unique" items={experience.unique_points}/><ExperienceInfo experience={experience}/><ExperienceIncluded experience={experience}/><EditorialSection eyebrow="Close by" title="Nearby attractions" items={experience.nearby_attractions}/><ExperienceTips experience={experience}/><ExperienceFooterCTA experience={experience} globalTravellers={globalTravellers} initialParticipants={initialParticipants} onInclude={onInclude} onRemove={onRemove} onParticipantsChange={onParticipantsChange} included={included}/><ExperienceRelated experiences={related} onOpen={onRelatedOpen}/></article>;
}
