...
export function ExperienceCard({experience,onOpen}:{experience:JourneyExperience;onOpen:()=>void}){
  return <motion.button layout onClick={onOpen} whileHover={{y:-6}} className="group overflow-hidden rounded-[2rem] bg-white text-left shadow-[0_24px_70px_rgba(26,40,35,.09)] transition-shadow hover:shadow-[0_30px_90px_rgba(26,40,35,.16)]">
    ...
    <div className="p-6 md:p-8">
      ...
      <h2 className="mt-4 font-serif text-3xl leading-tight text-slate md:text-[2.15rem]">{experience.name}</h2>
...
function Counter({label,value,onChange}:{label:string;value:number;onChange:(value:number)=>void}){
  return <div className="flex items-center justify-between border-b border-white/15 py-4"><span>{label}</span><div className="flex items-center gap-4"><button type="button" onClick={()=>onChange(Math.max(0,value-1))} className="grid size-9 place-items-center rounded-full border border-white/25" aria-label={`Remove one ${label}`}><Minus className="size-4"/></button><strong className="w-5 text-center">{value}</strong><button type="button" onClick={()=>onChange(value+1)} className="grid size-9 place-items-center rounded-full border border-white/25" aria-label={`Add one ${label}`}><Plus className="size-4"/></button></div></div>;
}
...
export function ExperienceParticipants({counts,onChange,dark=true}:{counts:ParticipantCounts;onChange:(counts:ParticipantCounts)=>void;dark?:boolean}){
  return <div className={cn("rounded-[1.75rem] p-6",dark?"bg-white/8":"bg-sand-light")}><p className="mb-2 text-xs font-bold uppercase tracking-[.18em] opacity-60">Participants</p>{(["adults","children","infants"] as const).map(key=><Counter key={key} label={key[0].toUpperCase()+key.slice(1)} value={counts[key]} onChange={value=>onChange({...counts,[key]:value})}/>)}</div>;
}

function JourneyTravellersModal({open,initial,onCancel,onConfirm}:{open:boolean;initial:ParticipantCounts;onCancel:()=>void;onConfirm:(counts:ParticipantCounts)=>void}){
  const [counts,setCounts]=useState(initial);
  useEffect(()=>{if(open)setCounts(initial.adults?initial:{adults:1,children:0,infants:0})},[initial,open]);
  return <AnimatePresence>{open&&<motion.div className="fixed inset-0 z-[90] grid place-items-center bg-slate/70 p-4 backdrop-blur-sm" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={onCancel}><motion.div role="dialog" aria-modal="true" aria-labelledby="travellers-title" onMouseDown={event=>event.stopPropagation()} initial={{opacity:0,y:24,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:14}} className="w-full max-w-lg rounded-[2rem] bg-ivory p-7 shadow-2xl md:p-10"><p className="eyebrow">Your journey</p><h2 id="travellers-title" className="mt-3 font-serif text-4xl">Who will be joining this journey?</h2><p className="mt-3 text-sm leading-6 text-stone">Set the overall travelling party once. You can still choose a different participant count for each experience.</p><div className="mt-7 rounded-2xl bg-forest p-5 text-ivory"><ExperienceParticipants counts={counts} onChange={setCounts}/></div><div className="mt-7 flex justify-end gap-3"><Button variant="ghost" onClick={onCancel}>Not now</Button><Button disabled={total(counts)===0} onClick={()=>onConfirm(counts)}>Continue</Button></div></motion.div></motion.div>}</AnimatePresence>;
}

export function ExperienceFooterCTA({experience,globalTravellers=emptyCounts,initialParticipants,onInclude,included=false}:{experience:JourneyExperience;globalTravellers?:ParticipantCounts;initialParticipants?:ParticipantCounts;onInclude?:(participants:ParticipantCounts,journeyTravellers:ParticipantCounts)=>void;included?:boolean}){
  ...
  const include=()=>{if(!total(globalTravellers)){setAskingTravellers(true);return}if(total(participants))onInclude?.(participants,globalTravellers)};
  return <section className="bg-forest py-16 text-ivory md:py-24"><div className="shell grid items-center gap-10 md:grid-cols-[1.1fr_.9fr]"><div><p className="eyebrow text-gold-light">Make it part of your story</p><h2 className="mt-4 max-w-xl font-serif text-4xl leading-tight md:text-6xl">Design my journey with this experience.</h2><p className="mt-5 max-w-xl leading-8 text-ivory/65">Choose who will take part. This can be different from the total number travelling with you.</p></div><div><ExperienceParticipants counts={participants} onChange={setParticipants}/><Button variant="accent" size="lg" className="mt-6 w-full" disabled={included||total(participants)===0&&total(globalTravellers)>0} onClick={include}>{included?<><Check/>Included in your journey</>:"Include this experience in my journey"}</Button></div></div><JourneyTravellersModal open={askingTravellers} initial={globalTravellers} onCancel={()=>setAskingTravellers(false)} onConfirm={counts=>{setAskingTravellers(false);const next=total(participants)?participants:counts;setParticipants(next);onInclude?.(next,counts)}}/></section>;
}
...
export function ExperienceDiscovery({experiences,globalTravellers=emptyCounts,selectedIds=[],participantsByExperience={},onInclude}:{experiences:JourneyExperience[];globalTravellers?:ParticipantCounts;selectedIds?:string[];participantsByExperience?:Record<string,ParticipantCounts>;onInclude?:(experience:JourneyExperience,participants:ParticipantCounts,journeyTravellers:ParticipantCounts)=>void}){
  ...
  return <><motion.div layout className="mt-10 grid gap-7 md:grid-cols-2">{experiences.map(experience=><ExperienceCard key={experience.id} experience={experience} onOpen={()=>setActive(experience)}/>)}</motion.div><AnimatePresence>{active&&<motion.div className="fixed inset-0 z-[70] overflow-y-auto bg-ivory" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><DetailContents experience={active} related={related} onClose={()=>setActive(null)} globalTravellers={globalTravellers} initialParticipants={participantsByExperience[active.id]} included={selectedIds.includes(active.id)} onInclude={(participants,journeyTravellers)=>onInclude?.(active,participants,journeyTravellers)} onRelatedOpen={setActive}/></motion.div>}</AnimatePresence></>;
}

export function ExperienceEditorialPage({experience,related}:{experience:JourneyExperience;related:JourneyExperience[]}){
  const [globalTravellers,setGlobalTravellers]=useState<ParticipantCounts>(emptyCounts);
  const include=(participants:ParticipantCounts,journeyTravellers:ParticipantCounts)=>{
    setGlobalTravellers(journeyTravellers);
    const query=new URLSearchParams({experience:experience.id,adults:String(journeyTravellers.adults),children:String(journeyTravellers.children),infants:String(journeyTravellers.infants),experienceAdults:String(participants.adults),experienceChildren:String(participants.children),experienceInfants:String(participants.infants)});
    window.location.assign(`/journey-builder?${query.toString()}`);
  };
  return <><div className="shell py-5"><Link href="/experiences" className="inline-flex items-center gap-2 text-sm font-semibold text-slate/65 hover:text-slate"><ArrowLeft className="size-4"/>All experiences</Link></div><DetailContents experience={experience} related={related} globalTravellers={globalTravellers} onInclude={include}/></>;
}
