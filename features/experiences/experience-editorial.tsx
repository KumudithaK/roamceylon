...
import {PricingPlanChoices} from "@/components/journey/pricing-plan-choices";
...
export function ExperienceCard({experience,onOpen,onRemove,compact=false,selected=false}:{experience:JourneyExperience;onOpen:()=>void;onRemove?:()=>void;compact?:boolean;selected?:boolean}){
  return <motion.article layout whileHover={{y:-6}} className={cn("group overflow-hidden rounded-[2rem] border-2 bg-white text-left transition",selected?"border-gold shadow-[0_24px_70px_rgba(193,140,45,.2)]":"border-transparent shadow-[0_24px_70px_rgba(26,40,35,.09)] hover:shadow-[0_30px_90px_rgba(26,40,35,.16)]")}>
    <button type="button" onClick={onOpen} className="block w-full text-left">
      ...
      {experience.hero_image_url&&<Image src={experience.hero_image_url} alt={experience.image_alt||experience.name} fill sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" className="object-cover transition duration-1000 ease-out group-hover:scale-[1.045]"/>}
...
export function ExperienceGallery({experience}:{experience:JourneyExperience}){
  ...
  return <section className="shell py-16 md:py-24"><p className="eyebrow mb-5">In pictures</p><div className="grid auto-rows-[210px] gap-4 md:grid-cols-12 md:auto-rows-[260px]">{images.slice(0,5).map((image,index)=><div key={image} className={cn("relative overflow-hidden rounded-[1.75rem] bg-sand",index===0?"md:col-span-7 md:row-span-2":"md:col-span-5")}><Image src={image} alt={experience.gallery_alt_texts[index]||`${experience.name} — view ${index+1}`} fill sizes={index===0?"60vw":"40vw"} className="object-cover transition duration-700 hover:scale-[1.03]"/></div>)}</div>{experience.image_credit&&<p className="mt-4 max-w-5xl text-xs leading-5 text-stone/70">Image credits: {experience.image_credit}</p>}</section>;
}
...
export function ExperienceFooterCTA({experience,globalTravellers=emptyCounts,initialParticipants,initialPricingPlanId,onInclude,onParticipantsChange,included=false}:{experience:JourneyExperience;globalTravellers?:ParticipantCounts;initialParticipants?:ParticipantCounts;initialPricingPlanId?:string;onInclude?:(participants:ParticipantCounts,journeyTravellers:ParticipantCounts,pricingPlanId?:string)=>void;onParticipantsChange?:(participants:ParticipantCounts,pricingPlanId?:string)=>void;included?:boolean}){
  ...
  const [pricingPlanId,setPricingPlanId]=useState(initialPricingPlanId||experience.pricingPlans[0]?.id||"");
  ...
  useEffect(()=>{setPricingPlanId(initialPricingPlanId||experience.pricingPlans[0]?.id||"")},[experience.pricingPlans,initialPricingPlanId]);
  const validForTrip=!total(globalTravellers)||withinTrip(participants,globalTravellers);
  const hasRequiredPlan=!experience.pricingPlans.length||Boolean(pricingPlanId);
  const include=()=>{if(!total(globalTravellers)){setAskingTravellers(true);return}if(total(participants)&&validForTrip&&hasRequiredPlan)onInclude?.(participants,globalTravellers,pricingPlanId||undefined)};
  const changeParticipants=(counts:ParticipantCounts)=>{setUpdated(false);setParticipants(counts)};
  const updateParticipants=()=>{if(!validForTrip||!total(participants)||!hasRequiredPlan)return;onParticipantsChange?.(participants,pricingPlanId||undefined);setUpdated(true)};
  return <section className="bg-forest py-16 text-ivory md:py-24"><div className="shell grid items-center gap-10 md:grid-cols-[1.1fr_.9fr]"><div><p className="eyebrow text-gold-light">Make it part of your story</p><h2 className="mt-4 max-w-xl font-serif text-4xl leading-tight md:text-6xl">{included?"Update experience participants.":"Design my journey with this experience."}</h2><p className="mt-5 max-w-xl leading-8 text-ivory/65">{included?"Adjust who will take part and choose the applicable rate, then update your journey. You can remove this experience directly from its selected card.":"Choose who will take part and the experience rate that suits your journey."}</p></div><div><ExperienceParticipants counts={participants} onChange={changeParticipants}/>{!validForTrip&&<p role="alert" className="mt-4 rounded-xl border border-gold-light/30 bg-gold/10 p-4 text-sm leading-6 text-gold-light">Experience participants cannot exceed your whole-trip travellers. Reduce these participants or increase the traveller count in the Plan step.</p>}<PricingPlanChoices plans={experience.pricingPlans} value={pricingPlanId} onChange={id=>{setUpdated(false);setPricingPlanId(id)}} dark/><Button variant={included?"outline":"accent"} size="lg" className={cn("mt-6 w-full",included&&"border-gold-light/50 text-gold-light hover:bg-white/10")} disabled={total(participants)===0||!validForTrip||!hasRequiredPlan} onClick={included?updateParticipants:include}>{included?"Update experience selection":"Include this experience in my journey"}</Button>{included&&<p aria-live="polite" className={cn("mt-3 text-center text-xs font-semibold transition",updated?"text-gold-light":"text-ivory/45")}>{updated?<><Check className="mr-1 inline size-4"/>Experience selection updated in your journey.</>:"Changes are saved when you select Update experience selection."}</p>}</div></div><JourneyTravellersModal open={askingTravellers} initial={globalTravellers} minimum={participants} onCancel={()=>setAskingTravellers(false)} onConfirm={counts=>{setAskingTravellers(false);const next=total(participants)?participants:counts;setParticipants(next);onInclude?.(next,counts,pricingPlanId||undefined)}}/></section>;
}
...
function DetailContents({experience,related,onClose,globalTravellers,initialParticipants,initialPricingPlanId,onInclude,onParticipantsChange,included,onRelatedOpen}:{experience:JourneyExperience;related:JourneyExperience[];onClose?:()=>void;globalTravellers?:ParticipantCounts;initialParticipants?:ParticipantCounts;initialPricingPlanId?:string;onInclude?:(participants:ParticipantCounts,journeyTravellers:ParticipantCounts,pricingPlanId?:string)=>void;onParticipantsChange?:(participants:ParticipantCounts,pricingPlanId?:string)=>void;included?:boolean;onRelatedOpen?:(experience:JourneyExperience)=>void}){
  return <article className="bg-ivory text-slate"><ExperienceHero experience={experience} onClose={onClose}/><ExperienceStory experience={experience}/><ExperienceGallery experience={experience}/><ExperienceHighlights experience={experience}/><EditorialSection eyebrow="Distinctly this place" title="What makes it unique" items={experience.unique_points}/><ExperienceInfo experience={experience}/><ExperienceIncluded experience={experience}/><EditorialSection eyebrow="Close by" title="Nearby attractions" items={experience.nearby_attractions}/><ExperienceTips experience={experience}/><ExperienceFooterCTA experience={experience} globalTravellers={globalTravellers} initialParticipants={initialParticipants} initialPricingPlanId={initialPricingPlanId} onInclude={onInclude} onParticipantsChange={onParticipantsChange} included={included}/><ExperienceRelated experiences={related} onOpen={onRelatedOpen}/></article>;
}

export function ExperienceDiscovery({experiences,globalTravellers=emptyCounts,selectedIds=[],participantsByExperience={},selectedPricingPlanIds={},onInclude,onRemove,onParticipantsChange,compact=false}:{experiences:JourneyExperience[];globalTravellers?:ParticipantCounts;selectedIds?:string[];participantsByExperience?:Record<string,ParticipantCounts>;selectedPricingPlanIds?:Record<string,string>;onInclude?:(experience:JourneyExperience,participants:ParticipantCounts,journeyTravellers:ParticipantCounts,pricingPlanId?:string)=>void;onRemove?:(experience:JourneyExperience)=>void;onParticipantsChange?:(experience:JourneyExperience,participants:ParticipantCounts,pricingPlanId?:string)=>void;compact?:boolean}){
  ...
  return <><motion.div layout className={cn("mt-10 grid",compact?"gap-5 md:grid-cols-2":"gap-6 md:grid-cols-2 xl:grid-cols-3")}>{experiences.map(experience=>{const selected=selectedIds.includes(experience.id);return <ExperienceCard key={experience.id} experience={experience} compact={compact} selected={selected} onOpen={()=>setActive(experience)} onRemove={selected?()=>onRemove?.(experience):undefined}/>})}</motion.div><AnimatePresence>{active&&<motion.div className="fixed inset-0 z-[70] overflow-y-auto bg-ivory" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}><DetailContents experience={active} related={related} onClose={()=>setActive(null)} globalTravellers={globalTravellers} initialParticipants={participantsByExperience[active.id]} initialPricingPlanId={selectedPricingPlanIds[pricingPlanKey("experience",active.id)]} included={selectedIds.includes(active.id)} onInclude={(participants,journeyTravellers,pricingPlanId)=>{const selectedExperience=active;setActive(null);onInclude?.(selectedExperience,participants,journeyTravellers,pricingPlanId)}} onParticipantsChange={(participants,pricingPlanId)=>{const selectedExperience=active;setActive(null);onParticipantsChange?.(selectedExperience,participants,pricingPlanId)}} onRelatedOpen={setActive}/></motion.div>}</AnimatePresence></>;
}
...
export function ExperienceCatalogue({experiences}:{experiences:JourneyExperience[]}){
  ...
  const include=(experience:JourneyExperience,participants:ParticipantCounts,travellers:ParticipantCounts,pricingPlanId?:string)=>{
    persist(includeExperienceSelection(journey,experience,participants,travellers,pricingPlanId));
  ...
  return <ExperienceDiscovery experiences={experiences} globalTravellers={journey.travellerCounts} selectedIds={journey.selectedExperienceIds} participantsByExperience={journey.experienceParticipants} selectedPricingPlanIds={journey.selectedPricingPlanIds} onInclude={include} onRemove={experience=>persist(removeExperienceSelection(journey,experience.id))} onParticipantsChange={(experience,participants,pricingPlanId)=>{persist({...journey,experienceParticipants:{...journey.experienceParticipants,[experience.id]:participants},selectedPricingPlanIds:pricingPlanId?{...journey.selectedPricingPlanIds,[pricingPlanKey("experience",experience.id)]:pricingPlanId}:journey.selectedPricingPlanIds});window.location.assign("/journey-builder?step=2")}}/>;
}
...
export function ExperienceEditorialPage({experience,related}:{experience:JourneyExperience;related:JourneyExperience[]}){
  ...
  const include=(participants:ParticipantCounts,journeyTravellers:ParticipantCounts,pricingPlanId?:string)=>{
    const next=includeExperienceSelection(journey,experience,participants,journeyTravellers,pricingPlanId);
    persist(next);
  ...
  const updateParticipants=(participants:ParticipantCounts,pricingPlanId?:string)=>{
