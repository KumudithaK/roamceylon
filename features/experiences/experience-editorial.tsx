...
import {CalendarDays,Check,Clock,MapPin,Minus,Plus,Sparkles,X} from "lucide-react";
...
export function ExperienceFooterCTA({experience,globalTravellers=emptyCounts,initialParticipants,initialPricingPlanId,onInclude,onParticipantsChange,included=false}:{experience:JourneyExperience;globalTravellers?:ParticipantCounts;initialParticipants?:ParticipantCounts;initialPricingPlanId?:string;onInclude?:(participants:ParticipantCounts,journeyTravellers:ParticipantCounts,pricingPlanId?:string)=>void;onParticipantsChange?:(participants:ParticipantCounts,pricingPlanId?:string)=>void;included?:boolean}){
  ...
  return <section className="bg-forest py-16 text-ivory md:py-24"><div className="shell grid items-center gap-10 md:grid-cols-[1.1fr_.9fr]"><div><p className="eyebrow text-gold-light">Make it part of your story</p><h2 className="mt-4 max-w-xl font-serif text-4xl leading-tight md:text-6xl">{included?"Refine who will take part.":"Plan Your Journey"}</h2><p className="mt-5 max-w-xl leading-8 text-ivory/65">{included?"Adjust who will take part and choose the applicable rate, then update your journey. You can remove this experience directly from its selected card.":"Choose who will take part, then carry this experience into the wider journey you are shaping. Every arrangement remains subject to confirmation."}</p></div><div><ExperienceParticipants counts={participants} onChange={changeParticipants}/>{!validForTrip&&<p role="alert" className="mt-4 rounded-xl border border-gold-light/30 bg-gold/10 p-4 text-sm leading-6 text-gold-light">Experience participants cannot exceed your whole-trip travellers. Reduce these participants or increase the traveller count in the Plan step.</p>}<PricingPlanChoices plans={experience.pricingPlans} value={pricingPlanId} onChange={id=>{setUpdated(false);setPricingPlanId(id)}} dark/><Button variant={included?"outline":"accent"} size="lg" className={cn("mt-6 w-full",included&&"border-gold-light/50 text-gold-light hover:bg-white/10")} disabled={total(participants)===0||!validForTrip||!hasRequiredPlan} onClick={included?updateParticipants:include}>{included?"Update experience selection":"Plan Your Journey"}</Button>{included&&<p aria-live="polite" className={cn("mt-3 text-center text-xs font-semibold transition",updated?"text-gold-light":"text-ivory/45")}>{updated?<><Check className="mr-1 inline size-4"/>Experience selection updated in your journey.</>:"Changes are saved when you select Update experience selection."}</p>}</div></div><JourneyTravellersModal open={askingTravellers} initial={globalTravellers} minimum={participants} onCancel={()=>setAskingTravellers(false)} onConfirm={counts=>{setAskingTravellers(false);const next=total(participants)?participants:counts;setParticipants(next);onInclude?.(next,counts,pricingPlanId||undefined)}}/></section>;
}
...
function DetailContents({experience,related,onClose,globalTravellers,initialParticipants,initialPricingPlanId,onInclude,onParticipantsChange,included,onRelatedOpen}:{experience:JourneyExperience;related:JourneyExperience[];onClose?:()=>void;globalTravellers?:ParticipantCounts;initialParticipants?:ParticipantCounts;initialPricingPlanId?:string;onInclude?:(participants:ParticipantCounts,journeyTravellers:ParticipantCounts,pricingPlanId?:string)=>void;onParticipantsChange?:(participants:ParticipantCounts,pricingPlanId?:string)=>void;included?:boolean;onRelatedOpen?:(experience:JourneyExperience)=>void}){
  return <article className="bg-ivory text-slate"><ExperienceHero experience={experience} onClose={onClose}/><ExperienceStory experience={experience}/><ExperienceGallery experience={experience}/><ExperienceHighlights experience={experience}/><EditorialSection eyebrow="Distinctly this place" title="What makes it unique" items={experience.unique_points}/><ExperienceInfo experience={experience}/><ExperienceIncluded experience={experience}/><EditorialSection eyebrow="Close by" title="Nearby attractions" items={experience.nearby_attractions}/><ExperienceTips experience={experience}/><ExperienceFooterCTA key={experience.id} experience={experience} globalTravellers={globalTravellers} initialParticipants={initialParticipants} initialPricingPlanId={initialPricingPlanId} onInclude={onInclude} onParticipantsChange={onParticipantsChange} included={included}/><ExperienceRelated experiences={related} onOpen={onRelatedOpen}/></article>;
}
