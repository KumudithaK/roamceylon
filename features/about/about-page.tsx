...
const storyImage="https://upload.wikimedia.org/wikipedia/commons/7/77/Galle_Fort.jpg";
...
export function AboutPage({email,telephone}:{email:string;telephone:string}){
  const displayUrl="theroamceylon.com";
  return <>
    <section className="relative isolate flex min-h-[calc(100svh-7rem)] items-end overflow-hidden bg-slate text-ivory">
      ...
      <div className="shell relative z-10 py-20 md:py-28">
        <FadeIn className="max-w-4xl">
          ...
          <p className="mt-8 max-w-2xl text-base leading-8 text-ivory/78 md:text-lg">Every Roam Ceylon journey begins as a handful of wishes, then takes shape slowly—place by place, story by story—until it feels unmistakably yours.</p>
    ...
    <section className="section overflow-hidden">
      <div className="shell grid items-center gap-12 lg:grid-cols-[.88fr_1.12fr] lg:gap-24">
        <FadeIn className="relative min-h-[480px] overflow-hidden rounded-[2rem] bg-sand md:min-h-[650px]"><Image src={storyImage} alt="The sun-warmed ramparts and living heritage of Galle Fort" fill sizes="(max-width: 1024px) 100vw, 45vw" className="object-cover transition duration-1000 hover:scale-[1.025]"/><div className="absolute inset-0 bg-gradient-to-t from-slate/25 to-transparent"/></FadeIn>
        <FadeIn delay={.08}><p className="eyebrow mb-5">The Roam Ceylon story</p><h2 className="heading">Nothing this personal should ever feel taken from a shelf.</h2><div className="prose-luxury mt-8 space-y-6"><p>Sri Lanka unfurls in layers: ancient stone and temple bells, tea hills wrapped in morning mist, forest paths, salt-bright coasts and tables generous with spice. The island changes its mood within hours, and a beautiful journey must be allowed to breathe with it.</p><p>Roam Ceylon was born from a simple belief: travellers should begin with what stirs them, while people who know the island shape those wishes into a route with rhythm, warmth and meaning.</p><p>Each journey is freshly composed. Our Journey Designers listen, gather, arrange and refine—pairing the places you long to see with the pauses, encounters and quiet surprises that make the story entirely your own.</p></div></FadeIn>
      </div>
    </section>
    ...
    <section className="relative overflow-hidden bg-forest py-24 text-ivory md:py-36">
      ...
      <div className="shell relative grid items-center gap-14 lg:grid-cols-[1.2fr_.8fr] lg:gap-24">
        <FadeIn><p className="eyebrow mb-6 text-gold-light">The house in which our journeys are made</p><h2 className="heading">Roam Ceylon rests within Roam Ceylon Atelier (Private) Limited.</h2><div className="mt-8 max-w-3xl space-y-6 text-base leading-8 text-ivory/72 md:text-lg"><p>The Atelier is our creative home in Sri Lanka—the place where local knowledge, thoughtful hospitality and a traveller&apos;s wishes are gathered into one beautifully considered whole.</p><div className="pt-3"><p className="eyebrow mb-4 text-gold-light">Why &ldquo;Atelier&rdquo;?</p><p>An atelier is a maker&apos;s room: a place for patient hands, discerning eyes and work created for one person rather than many. That is how we approach a journey—never mass-produced, never hurried, and never quite the same twice.</p></div><p><strong className="font-semibold text-ivory">Roam Ceylon</strong> is the travelling expression of that house. Here, every route is imagined, tasted, adjusted and polished until it carries the traveller&apos;s own character.</p></div><p className="mt-12 border-l border-gold pl-7 font-serif text-3xl leading-tight text-ivory md:text-5xl">Freshly imagined. Carefully refined. Yours entirely.</p></FadeIn>
        <FadeIn delay={.1} className="relative grid min-h-[460px] place-items-center overflow-hidden rounded-[12rem_12rem_1.75rem_1.75rem] border border-gold/30 bg-[#f3ead8] p-12 text-center text-forest"><div className="absolute inset-0 opacity-30 [background-image:radial-gradient(circle_at_center,#c58f2f_1px,transparent_1px)] [background-size:22px_22px]"/><div className="relative"><div className="relative mx-auto size-52"><Image src="/assets/logo/roam-ceylon-elephant-transparent.png" alt="Roam Ceylon elephant emblem" fill sizes="208px" className="object-contain"/></div><p className="eyebrow mt-8">Made in Sri Lanka</p><p className="mt-4 font-serif text-3xl leading-tight">One island.<br/>One traveller.<br/>One story at a time.</p></div></FadeIn>
      </div>
    </section>
