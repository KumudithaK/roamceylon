...
export function PartnerApplicationForm({initialType}:{initialType:PartnerType}){
  ...
  const submit=async()=>{
    ...
    setSubmitting(false);
    if(!response.ok){const invalid=result.fields?Object.keys(result.fields).filter(key=>result.fields[key]?.length):[];setError(invalid.length?`Please check: ${invalid.map(labelFor).join(", ")}.`:result.error||"Application could not be submitted.");return}
      ...
      <div className="shell" aria-labelledby="partner-form-title">
        ...
        <div className="mt-8 grid overflow-hidden border border-forest/20 bg-surface lg:grid-cols-[.38fr_1fr]">
          ...
          <section aria-label={`${steps[step].label} information`} className="p-6 md:p-10">
            {step===0?<Grid><Select label="Partner type" value={type} required onChange={value=>{setType(value as PartnerType);setEntries([])}} options={[["accommodation","Accommodation"],["vehicle","Vehicle or fleet"],["guide","Local guide"]]}/><Text label="Full name" required value={values.applicantName} onChange={value=>set("applicantName",value)}/><Text label="Business or professional name" required value={values.businessName} onChange={value=>set("businessName",value)}/><Text label="Email" required type="email" value={values.email} onChange={value=>set("email",value)}/><div className="grid grid-cols-[6rem_1fr] gap-3"><Text label="Code" value={values.countryCode} onChange={value=>set("countryCode",value)}/><Text label="Phone / WhatsApp" required value={values.phone} onChange={value=>set("phone",value)}/></div><Select label="Preferred contact" required value={values.preferredContactMethod} onChange={value=>set("preferredContactMethod",value)} options={[["whatsapp","WhatsApp"],["email","Email"],["phone","Phone"]]}/><input tabIndex={-1} autoComplete="off" aria-hidden="true" className="hidden" value={String(values.honeypot)} onChange={event=>set("honeypot",event.target.value)}/></Grid>:null}

            {step===1?<Grid><Area label="Full address" required value={values.address} onChange={value=>set("address",value)}/><Select label="District" required value={values.district} onChange={value=>set("district",value)} options={districts.map(item=>[item,item])}/><Select label="Province" required value={values.province} onChange={value=>set("province",value)} options={provinces.map(item=>[item,item])}/><Text label="Website" hint="Optional" type="url" value={values.website} onChange={value=>set("website",value)}/><Text label="Social media page" hint="Optional" type="url" value={values.socialUrl} onChange={value=>set("socialUrl",value)}/><Area label="Short introduction" required value={values.introduction} onChange={value=>set("introduction",value)}/><Select label="How did you hear about us?" required value={values.heardFrom} onChange={value=>set("heardFrom",value)} options={["Recommendation","Search engine","Social media","Tourism network","The Ceylon Edition team","Other"].map(item=>[item,item])}/></Grid>:null}
...
function Area({label,value,required=false,onChange}:{label:string;value:string|boolean;required?:boolean;onChange:(value:string)=>void}){return <label className="grid gap-2 md:col-span-2"><FieldLabel label={label} required={required}/><textarea rows={4} required={required} aria-describedby={required?"partner-required-note":undefined} value={String(value)} onChange={event=>onChange(event.target.value)} className={fieldClass}/></label>}
...
function Uploads({title,icon:Icon,accept,files,setFiles,max,help}:{title:string;icon:typeof ImagePlus;accept:string;files:File[];setFiles:(files:File[])=>void;max:number;help:string}){return <section><div className="flex items-center gap-3"><Icon className="size-5 text-gold" aria-hidden="true"/><div><h3 className="font-serif text-2xl">{title}</h3><p className="mt-1 text-xs text-muted">{help}</p></div></div><label className="mt-4 grid min-h-28 cursor-pointer place-items-center border border-dashed border-forest/35 bg-ivory p-7 text-center text-sm font-semibold hover:border-gold"><input type="file" multiple accept={accept} className="sr-only" onChange={event=>setFiles([...files,...Array.from(event.target.files||[])].slice(0,max))}/><span>Choose files</span></label><div className="mt-3 grid gap-2">{files.map((file,index)=><div key={`${file.name}-${index}`} className="flex items-center justify-between gap-4 bg-sand/55 px-4 py-3 text-xs"><span className="truncate">{file.name} · {(file.size/1024/1024).toFixed(1)} MB</span><button type="button" onClick={()=>setFiles(files.filter((_,itemIndex)=>itemIndex!==index))} className="min-h-11 shrink-0 font-semibold text-red-800">Remove</button></div>)}</div></section>}
