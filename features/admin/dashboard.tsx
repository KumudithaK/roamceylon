...
import {BarChart3,BedDouble,CarFront,Handshake,LayoutDashboard,LogOut,Map,Settings,Sparkles,UsersRound} from "lucide-react";
...
export function AdminDashboard(){
  ...
  useEffect(()=>{const supabase=createClient();void (async()=>{
    const {data:{session}}=await supabase.auth.getSession();if(!session){router.replace("/admin/login");return}
    const tables=["themes","destinations","experiences","accommodations","vehicles","guides"] satisfies Array<keyof Database["public"]["Tables"]>;
    const statuses=["submitted","under_review","needs_information","approved","converted"] as const;
    const [values,partnerValues]=await Promise.all([
      Promise.all(tables.map(async table=>(await supabase.from(table).select("*",{count:"exact",head:true})).count||0)),
      Promise.all(statuses.map(async status=>(await supabase.from("partner_applications").select("*",{count:"exact",head:true}).eq("status",status)).count||0))
    ]);
    const contentMetrics=tables.map((label,index)=>({label:label.replaceAll("_"," "),value:values[index]}));
    const partnerMetrics=statuses.map((status,index)=>({label:status==="submitted"?"new partner applications":status==="under_review"?"applications needing review":status==="needs_information"?"awaiting information":`${status} applications`,value:partnerValues[index]}));
    setMetrics([...contentMetrics,...partnerMetrics]);setLoading(false);
  })()},[router]);
