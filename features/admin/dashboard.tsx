...
export function AdminDashboard(){
  ...
  const pipelineValue=enquiries.filter(row=>!["completed","cancelled","archived"].includes(row.status)).reduce((total,row)=>{
    ...
    return total+(!isJourneyEstimate(quote)&&quote?.status==="ready"&&quote.totalPackagePrice?quote.totalPackagePrice:0);
  },0);
  ...
  return <AdminShell><div className="mx-auto max-w-[1500px]">
    <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow mb-3">Roam Ceylon command centre</p><h1 className="font-serif text-4xl md:text-5xl">Welcome back.</h1><p className="mt-3 text-sm text-slate/55">Live commercial activity, traveller demand and publishing readiness.</p></div><div className="flex flex-wrap gap-3"><Link href="/admin/accounting" className="rounded-full border border-forest/20 bg-white px-5 py-3 text-sm font-bold text-forest">Open accounting</Link><Link href="/admin/enquiries" className="rounded-full bg-gold px-5 py-3 text-sm font-bold text-slate">Open enquiry inbox</Link><Link href="/admin/partner-applications" className="rounded-full bg-forest px-5 py-3 text-sm font-bold text-white">Review partners</Link></div></div>
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[1.3fr_.7fr]">
      <div className="overflow-hidden rounded-3xl border border-stone/15 bg-white"><div className="flex items-center justify-between border-b border-stone/15 p-7"><div><p className="eyebrow mb-2">Latest demand</p><h2 className="font-serif text-2xl">Recent traveller enquiries</h2></div><Link href="/admin/enquiries" className="flex items-center gap-1 text-sm font-semibold text-forest">View all <ArrowUpRight className="size-4"/></Link></div>{enquiries.length?<div className="divide-y divide-stone/15">{enquiries.slice(0,5).map(row=>{
        ...
        const estimateLabel=isJourneyEstimate(quote)&&quote.status==="estimated_range"?`${quote.currency} ${quote.perPersonMin?.toLocaleString("en-US",{maximumFractionDigits:0})}–${quote.perPersonMax?.toLocaleString("en-US",{maximumFractionDigits:0})} pp`:!isJourneyEstimate(quote)&&quote?.status==="ready"?`${quote.currency} ${quote.totalPackagePrice?.toLocaleString("en-US",{maximumFractionDigits:0})}`:"Personal quote";
        return <Link key={row.id} href={`/admin/enquiries/${row.id}`} className="grid gap-3 p-5 transition hover:bg-sand-light md:grid-cols-[1fr_160px_130px] md:items-center"><div><div className="flex flex-wrap items-center gap-2"><strong>{row.name}</strong><span className="rounded-full bg-gold/10 px-2 py-1 text-[.65rem] font-bold uppercase text-gold">{enquiryStatusLabels[row.status]}</span></div><p className="mt-1 text-xs text-stone">{row.journey_reference} · {row.adults+row.children} travellers · {destinationCount} destinations · {row.nationality||"Country open"}</p></div><strong className="text-sm text-forest">{estimateLabel}</strong><span className="text-xs text-stone">{shortDate(row.created_at)}</span></Link>;
