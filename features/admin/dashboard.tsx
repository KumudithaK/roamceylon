...
export function AdminDashboard(){
  ...
  return <AdminShell requiredPermission="admin.dashboard.view"><div className="mx-auto max-w-[1500px]">
    <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow mb-3">The Ceylon Edition command centre</p><h1 className="font-serif text-4xl md:text-5xl">Welcome back.</h1><p className="mt-3 text-sm text-slate/55">Live activity from the parts of the atelier entrusted to your role.</p></div><div className="flex flex-wrap gap-3">{finance?<Link href="/admin/accounting" className="rounded-full border border-forest/20 bg-white px-5 py-3 text-sm font-bold text-forest">Open accounting</Link>:null}{journeys?<Link href="/admin/enquiries" className="rounded-full bg-gold px-5 py-3 text-sm font-bold text-slate">Open enquiry inbox</Link>:null}{partners?<Link href="/admin/partner-applications" className="rounded-full bg-forest px-5 py-3 text-sm font-bold text-white">Review partners</Link>:null}</div></div>
