...
type Enquiry=Database["public"]["Views"]["staff_journey_request_summary"]["Row"];
...
const money=(row:Enquiry)=>{
  if(row.estimated_price_min===null||row.estimated_price_max===null)return "Personal quotation";
  return `${row.estimated_price_currency??"USD"} ${row.estimated_price_min.toLocaleString("en-US",{maximumFractionDigits:0})}–${row.estimated_price_max.toLocaleString("en-US",{maximumFractionDigits:0})}${row.estimated_price_basis==="per_person"?" pp":""}`;
};
...
export function EnquiryInbox(){
  ...
  useEffect(()=>{void (async()=>{
    ...
    const {data,error}=await database.from("staff_journey_request_summary").select("*").order("created_at",{ascending:false});
    if(error){setLoadError(error.message);setLoading(false);return}
    setRows(data??[]);
    setLoading(false);
  })()},[router]);
  const visible=useMemo(()=>rows.filter(row=>(filter==="all"||row.status===filter)&&`${row.journey_reference} ${row.traveller_name??""}`.toLowerCase().includes(search.toLowerCase())),[rows,filter,search]);
  ...
  return <AdminShell requiredPermission="journey.requests.view"><div className="mx-auto max-w-7xl">
    ...
    <label className="mt-5 flex max-w-xl items-center gap-3 rounded-2xl border border-stone/15 bg-white px-4"><Search className="size-4 text-stone"/><input value={search} onChange={event=>setSearch(event.target.value)} placeholder="Search Journey ID or available traveller name…" className="w-full bg-transparent py-4 text-sm outline-none"/></label>
    <div className="mt-7 overflow-hidden rounded-3xl border border-stone/15 bg-white">{loading?<div className="h-72 animate-pulse bg-sand-light"/>:visible.length?<div className="divide-y divide-stone/15">{visible.map(row=>{
      ...
      return <Link key={row.id} href={`/admin/enquiries/${row.id}`} className="grid gap-4 p-5 transition hover:bg-sand-light lg:grid-cols-[1.3fr_.8fr_.7fr_.55fr] lg:items-center">
        <div><div className="flex flex-wrap items-center gap-2"><strong className="font-serif text-xl">{row.traveller_name??row.journey_reference}</strong><span className={`rounded-full px-2.5 py-1 text-[.65rem] font-bold uppercase ${statusStyle[row.status]}`}>{enquiryStatusLabels[row.status]}</span></div><p className="mt-1 text-[.68rem] font-bold uppercase tracking-widest text-gold">{row.journey_reference}</p></div>
        <div className="grid gap-1 text-sm"><span className="flex items-center gap-2"><Users className="size-4 text-gold"/>{travellers} traveller{travellers===1?"":"s"} · {row.destination_count} destination{row.destination_count===1?"":"s"}</span><span className="flex items-center gap-2 text-xs text-stone"><CalendarDays className="size-4"/>{row.travel_start_date||"Dates not selected"}</span></div>
