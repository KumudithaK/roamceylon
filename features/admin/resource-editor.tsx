...
function GuideEditor({record,set,tab,plans,id,destinations,themes,experiences,relationships,setRelationships}:EditorProps){
  ...
  if(tab==="coverage")return <div className="grid gap-7"><div className="rounded-2xl bg-sand-light p-5"><p className="font-serif text-xl text-slate">Guide coverage & specialist matching</p><p className="mt-2 text-sm leading-6 text-stone">Destination coverage identifies local and site guides. Experience matching identifies specialists for selected activities. These mappings create an internal shortlist; they never assign a guide automatically.</p></div><Toggle label="Available nationwide" value={record.nationwide} onChange={value=>set("nationwide",value)}/>{!record.nationwide?<RelationshipPicker label="Destinations" description="Places where this guide normally works as a local, area or site specialist." options={destinations} selected={relationships.destinationIds} onChange={destinationIds=>setRelationships(current=>({...current,destinationIds}))}/>:null}<RelationshipPicker label="Travel themes" description="Journey styles this guide has genuine specialist knowledge in." options={themes} selected={relationships.themeIds} onChange={themeIds=>setRelationships(current=>({...current,themeIds}))}/><RelationshipPicker label="Experiences" description="Specific experiences this guide is qualified or approved to lead. These links are used when shortlisting specialists for an enquiry." options={experiences} selected={relationships.experienceIds} onChange={experienceIds=>setRelationships(current=>({...current,experienceIds}))}/></div>;
