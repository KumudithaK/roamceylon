...
export function ResourceEditor({config,id}:{config:ResourceConfig;id:string}){
  ...
  const props:EditorProps={record,set:(key,value)=>setRecord({...record,[key]:value}),tab,plans,id,destinations};
...
function StayEditor({record,set,tab,plans,id,destinations}:EditorProps){
  ...
  if(tab==="policies")return <Grid><Area label="Address" value={record.address} onChange={value=>set("address",value)}/><Text label="Booking website" value={record.booking_url} onChange={value=>set("booking_url",value)}/></Grid>;
