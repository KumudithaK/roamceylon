...
export function QuotationModal({open,onClose,state,quote}:{open:boolean;onClose:()=>void;state:JourneyState;quote:PublicPackageQuote|null}){
  ...
  return <div role="dialog" aria-modal="true" aria-labelledby="quotation-title" className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-slate/70 p-4 backdrop-blur-sm" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}>
    <div className="relative my-6 w-full max-w-3xl overflow-hidden rounded-[2rem] bg-ivory shadow-2xl">
      ...
      {sent?<div className="grid min-h-[430px] place-items-center p-10 text-center"><div><CheckCircle2 className="mx-auto size-14 text-gold"/><p className="eyebrow mt-6">Request received</p><h2 id="quotation-title" className="mt-3 font-serif text-4xl">Your journey designer is on it.</h2><p className="mx-auto mt-4 max-w-lg text-slate/60">We have received your journey and contact details. A Roam Ceylon journey designer will review everything and contact you personally.</p><Button className="mt-8" onClick={onClose}>Return to my journey</Button></div></div>:
      <form onSubmit={handleSubmit(submit)}>
        <div className="bg-forest px-7 py-8 pr-20 text-ivory md:px-10"><p className="eyebrow text-gold-light">Private, tailor-made travel</p><h2 id="quotation-title" className="mt-2 font-serif text-3xl md:text-4xl">Request your personalised journey.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-ivory/65">Share your details and a Roam Ceylon journey designer will review your journey personally.</p></div>
        <div className="grid gap-5 p-7 md:grid-cols-2 md:p-10">
          ...
          <Button disabled={isSubmitting} type="submit" variant="accent" className="md:col-span-2">{isSubmitting?"Sending your journey…":"Send journey request"}</Button>
          <p className="text-center text-xs text-stone md:col-span-2">No payment is taken now. Your journey designer will contact you personally.</p>
        </div>
      </form>}
    </div>
  </div>;
