  ...
  const exportPdf=async()=>{
    ...
    try{
      await exportJourneyPdf({
        ...
        destinations:state.selectedDestinationIds.map(id=>data.destinations.find(item=>item.id===id)?.name).filter((name):name is string=>Boolean(name)),
        experiences:state.selectedExperienceIds.map(id=>data.experiences.find(item=>item.id===id)?.name).filter((name):name is string=>Boolean(name)),
        accommodations:selectedStays.map(item=>item.name),
        vehicle:vehicle?.listing_title||null,
