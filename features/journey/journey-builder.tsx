...
function PlanStep({data}:{data:JourneyBootstrap}){
  ...
  return <div className="mt-10">
    ...
    <div className="mt-5 grid gap-4">
      {planTab==="Stay"?(stays.length?stays.map(item=>{const selected=state.selectedStayIdsByDestination[item.destinationId]===item.id;return <button key={item.id} onClick={()=>dispatch({type:"stay",destinationId:item.destinationId,stayId:selected?"":item.id})} className={cn("rounded-2xl border bg-white p-5 text-left",selected?"border-gold ring-2 ring-gold/25":"border-stone/20")}><strong>{item.name}</strong><span className="block text-sm text-stone">{item.property_type}{item.nightly_rate_usd!==null?` · $${item.nightly_rate_usd}/night`:" · Price on request"}</span></button>}):<Empty text="No published stays are available for your selected destinations yet."/>):null}
      {planTab==="Getting Around"?(vehicles.length?vehicles.map(item=>{const selected=state.selectedVehicleId===item.id;return <button key={item.id} onClick={()=>dispatch({type:"vehicle",id:selected?null:item.id})} className={cn("rounded-2xl border bg-white p-5 text-left",selected?"border-gold ring-2 ring-gold/25":"border-stone/20")}><strong>{item.listing_title}</strong><span className="block text-sm text-stone">{item.vehicle_type} · {item.passenger_capacity||"Flexible"} guests{item.daily_rate_usd!==null?` · $${item.daily_rate_usd}/day`:" · Price on request"}</span></button>}):<Empty text="No published vehicle partners match this journey yet."/>):null}
      {planTab==="Local Guides"?(guides.length?guides.map(item=>{const selected=state.selectedGuideId===item.id;return <button key={item.id} onClick={()=>dispatch({type:"guide",id:selected?null:item.id})} className={cn("rounded-2xl border bg-white p-5 text-left",selected?"border-gold ring-2 ring-gold/25":"border-stone/20")}><strong>{item.name}</strong><span className="block text-sm text-stone">{item.languages.join(" · ")}{item.daily_rate_usd!==null?` · $${item.daily_rate_usd}/day`:" · Price on request"}</span></button>}):<Empty text="No matching published local guides are currently available."/>):null}
    </div>
