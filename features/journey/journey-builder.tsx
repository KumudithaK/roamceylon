...
import {JourneyProvider,useJourney,type JourneyInitialSelection} from "./journey-store";
...
function PlanSelectionModal({preview,selected,onClose,onConfirm}:{preview:PlanPreview|null;selected:boolean;onClose:()=>void;onConfirm:()=>void}){
  ...
  const details=kind==="stay"
    ?[item.property_type,item.amenities.length?item.amenities.slice(0,6).join(" · "):null,item.nightly_rate_usd!==null?`From $${item.nightly_rate_usd} per night`:"Final rate on request"]
    :kind==="vehicle"
      ?[item.vehicle_type,item.passenger_capacity?`${item.passenger_capacity} passengers`:null,item.luggage_capacity,item.driver_included?"Driver included":null,item.daily_rate_usd!==null?`From $${item.daily_rate_usd} per day`:"Final rate on request"]
      :[item.languages.length?item.languages.join(" · "):null,item.years_experience?`${item.years_experience} years of experience`:null,item.specialities.length?item.specialities.join(" · "):null,item.daily_rate_usd!==null?`From $${item.daily_rate_usd} per day`:"Final rate on request"];
...
function PlanStep({data}:{data:JourneyBootstrap}){
  ...
  return <div className="mt-10">
    ...
    <div className="mt-5 grid gap-4 md:grid-cols-2">
      {planTab==="Stay"?(stays.length?stays.map(item=>{const selected=state.selectedStayIdsByDestination[item.destinationId]===item.id;return <PlanOptionCard key={item.id} title={item.name} meta={`${item.property_type||"Accommodation"}${item.nightly_rate_usd!==null?` · $${item.nightly_rate_usd}/night`:" · Price on request"}`} image={item.hero_image_url} alt={item.image_alt} selected={selected} onClick={()=>setPreview({kind:"stay",item})}/>;}):<Empty text="No published stays are available for your selected destinations yet."/>):null}
      {planTab==="Getting Around"?(vehicles.length?vehicles.map(item=>{const selected=state.selectedVehicleId===item.id;return <PlanOptionCard key={item.id} title={item.listing_title} meta={`${item.vehicle_type||"Private transport"} · ${item.passenger_capacity||"Flexible"} guests${item.daily_rate_usd!==null?` · $${item.daily_rate_usd}/day`:" · Price on request"}`} image={item.hero_image_url} alt={item.image_alt} selected={selected} onClick={()=>setPreview({kind:"vehicle",item})}/>;}):<Empty text="No published vehicle partners match this journey yet."/>):null}
      {planTab==="Local Guides"?(guides.length?guides.map(item=>{const selected=state.selectedGuideId===item.id;return <PlanOptionCard key={item.id} title={item.name} meta={`${item.languages.join(" · ")||"Local guide"}${item.daily_rate_usd!==null?` · $${item.daily_rate_usd}/day`:" · Price on request"}`} image={item.profile_image_url} alt={item.image_alt} selected={selected} onClick={()=>setPreview({kind:"guide",item})}/>;}):<Empty text="No matching published local guides are currently available."/>):null}
    </div>
    ...
    <PlanSelectionModal preview={preview} selected={preview?.kind==="stay"?state.selectedStayIdsByDestination[preview.item.destinationId]===preview.item.id:preview?.kind==="vehicle"?state.selectedVehicleId===preview.item.id:preview?.kind==="guide"?state.selectedGuideId===preview.item.id:false} onClose={()=>setPreview(null)} onConfirm={()=>{if(!preview)return;if(preview.kind==="stay"){const selected=state.selectedStayIdsByDestination[preview.item.destinationId]===preview.item.id;dispatch({type:"stay",destinationId:preview.item.destinationId,stayId:selected?"":preview.item.id});}else if(preview.kind==="vehicle")dispatch({type:"vehicle",id:state.selectedVehicleId===preview.item.id?null:preview.item.id});else dispatch({type:"guide",id:state.selectedGuideId===preview.item.id?null:preview.item.id});setPreview(null);}}/>
  </div>;
}
...
function Summary({data,quoteState,onQuotation}:{data:JourneyBootstrap;quoteState:ReturnType<typeof usePackageQuote>;onQuotation:()=>void}){
  ...
  const exportPdf=async()=>{
    setExporting(true);
    try{await exportJourneyPdf({themes:state.selectedThemeIds.map(id=>data.themes.find(item=>item.id===id)?.name).filter((name):name is string=>Boolean(name)),destinations:state.selectedDestinationIds.map(id=>data.destinations.find(item=>item.id===id)?.name).filter((name):name is string=>Boolean(name)),experiences:state.selectedExperienceIds.map(id=>data.experiences.find(item=>item.id===id)?.name).filter((name):name is string=>Boolean(name)),accommodations:selectedStays.map(item=>item.name),vehicle:vehicle?.listing_title||null,guide:guide?.name||null,travelDates:state.travelDates,travellerCounts:state.travellerCounts,estimatedDistance:route.estimatedDistance,estimatedTravelDays:route.estimatedTravelDays,quote});}finally{setExporting(false)}
  };
  ...
  return <aside className="h-fit rounded-3xl bg-forest p-7 text-ivory lg:sticky lg:top-28">
    ...
    {[[state.selectedThemeIds,data.themes,"Themes"],[state.selectedDestinationIds,data.destinations,"Destinations"],[state.selectedExperienceIds,data.experiences,"Experiences"]].map(([ids,items,label])=><div className="border-t border-ivory/10 py-5" key={label as string}><p className="mb-3 text-[.65rem] font-bold uppercase tracking-widest text-gold-light">{label as string}</p><div className="flex flex-wrap gap-2">{(ids as string[]).length?(ids as string[]).map(id=><span key={id} className="rounded-full bg-ivory/10 px-3 py-1 text-xs">{(items as Card[]).find(item=>item.id===id)?.name}{label==="Experiences"&&state.experienceParticipants[id]?` · ${state.experienceParticipants[id].adults+state.experienceParticipants[id].children+state.experienceParticipants[id].infants}`:""}</span>):<span className="text-sm text-ivory/40">Nothing selected yet</span>}</div></div>)}
    <div className="border-t border-ivory/10 py-5"><p className="mb-3 text-[.65rem] font-bold uppercase tracking-widest text-gold-light">Plan</p><div className="grid gap-2 text-sm text-ivory/70">{selectedStays.map(item=><span key={item.id}>{item.name}</span>)}{vehicle&&<span>{vehicle.listing_title}</span>}{guide&&<span>{guide.name}</span>}{!selectedStays.length&&!vehicle&&!guide&&<span className="text-ivory/40">Nothing selected yet</span>}</div></div>
    ...
    <Button onClick={onQuotation} disabled={!state.selectedDestinationIds.length} variant="accent" className="mb-3 w-full">Request Final Quotation</Button>
