...
import {saveJourneyHandoff} from "@/lib/journey/quotation-handoff";
...
function Summary({data}:{data:JourneyBootstrap}){
  ...
  return <aside className="h-fit rounded-3xl bg-forest p-7 text-ivory lg:sticky lg:top-28">
    ...
    <Button asChild variant="accent" className={cn("mb-3 w-full",!state.selectedDestinationIds.length&&"pointer-events-none opacity-50")}><Link onClick={()=>saveJourneyHandoff(state,quote)} aria-disabled={!state.selectedDestinationIds.length} href={state.selectedDestinationIds.length?{pathname:"/contact",query:{journey:"1",quotation:"1"}}:"#"}>Request Final Quotation</Link></Button>
...
export function JourneyBuilder({data,initialSelection}:{data:JourneyBootstrap;initialSelection?:JourneyInitialSelection}){
  return <JourneyProvider data={data} initialSelection={initialSelection}><Builder data={data} beginAtExperience={Boolean(initialSelection?.experienceId)}/></JourneyProvider>;
}
