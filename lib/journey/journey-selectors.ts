...
export const availableExperiences=(items:JourneyExperience[],destinationIds:string[],themeIds:string[])=>destinationIds.length&&themeIds.length?unionById(items.filter(item=>item.destinationIds.some(id=>destinationIds.includes(id))&&item.themeIds.some(id=>themeIds.includes(id)))).map(item=>({...item,matchedDestinationIds:item.destinationIds.filter(id=>destinationIds.includes(id))})).sort((a,b)=>a.name.localeCompare(b.name)):[];
