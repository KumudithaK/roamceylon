...
const reportBootstrapFailure=(scope:string,error:unknown)=>{
  const classification=error instanceof Error?error.name:"UnavailableError";
  console.error(`[journey:${scope}] ${classification}`);
};
export class JourneyService{
  ...
  async getJourneyBootstrapData():Promise<JourneyBootstrap>{
    ...
    results.forEach((result,index)=>{if(result.status==="rejected")reportBootstrapFailure(["accommodations","vehicles","guides"][index],result.reason)});
