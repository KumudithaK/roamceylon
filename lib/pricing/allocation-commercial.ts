...
export function calculateAllocationCommercials(lines:AllocationCommercialLine[],config:AllocationCommercialConfig,context:{days:number;nights:number;distanceKm:number}):AllocationCommercialResult{
  ...
  const operations:Array<[string,string,number,boolean]>=[
    ["driver-salary","Driver operations",config.driverSalaryPerDay*days,vehicles.length>0&&!vehicleIncludes("driverIncluded")],
    ["fuel","Journey fuel",config.vehicleKmPerLitre>0?distance/config.vehicleKmPerLitre*config.fuelPricePerLitre:0,vehicles.length>0&&!vehicleIncludes("fuelIncluded")],
    ["tolls","Road tolls",config.tollsPerJourney,vehicles.length>0&&!vehicleIncludes("tollsIncluded")],
    ["parking","Journey parking",config.parkingPerDay*days,vehicles.length>0&&!vehicleIncludes("parkingIncluded")],
    ["guide-accommodation","Guide accommodation",config.guideAccommodationPerNight*nights,guides.length>0&&!guides.every(line=>flag(line.serviceDetails,"accommodationIncluded"))]
  ];
  ...
  const administrationFee=money(config.administrationFixed+direct*config.administrationPercent/100);
  const contingency=money((direct+administrationFee)*config.contingencyPercent/100);
